...
async function main() {
  ...
  const lecturerUser = await prisma.user.upsert({
    where: { email: 'feliks@uic.edu' },
    update: {},
    create: {
      email: 'feliks@uic.edu',
      name: 'Feliks (Lecturer)',
      role: 'LECTURER',
    },
  })

  const studentUser = await prisma.user.upsert({
    where: { email: 'student@uic.edu' },
    update: {},
    create: {
      email: 'student@uic.edu',
      name: 'Student Demo',
      role: 'STUDENT',
    },
  })
  ...
  const studio1 = await prisma.studio.upsert({
    ...
    create: {
      ...
      description: 'Professional recording studio equipped with high-end analog and digital gear for music production, mixing, and mastering.',
      location: null,
      capacity: null,
      images: null,
    },
  ...
  const studio2 = await prisma.studio.upsert({
    ...
    create: {
      ...
      description: 'Recording studio with digital mixer and modern equipment for band recording and live sessions.',
      location: null,
      capacity: null,
      images: null,
    },
  ...
  const studio3 = await prisma.studio.upsert({
    ...
    create: {
      ...
      description: 'Open creative space for individual practice, small ensemble rehearsals, and creative work.',
      location: null,
      capacity: null,
      images: null,
    },
  ...
  // Create equipment from UIC Music Inventory
  const cables = [
  ...
  console.log(`✅ Equipment created (${allEquipment.length} items)`)
  ...
  const booking1 = await prisma.booking.create({
    data: {
      ...
      endDate: new Date(now.setHours(12, 0, 0, 0)),
      purpose: 'Recording demo project for final assignment',
      status: 'APPROVED',
      approvedById: lecturerUser.id,
      equipment: {
        create: [
          {
            equipmentId: (await prisma.equipment.findFirst({ where: { name: 'SENNHEISER HD 206 Headphone' } }))!.id,
            quantity: 2,
          },
          {
            equipmentId: (await prisma.equipment.findFirst({ where: { name: 'Shure SM57' } }))!.id,
            quantity: 1,
          },
        ],
      },
  ...
  const booking2 = await prisma.booking.create({
    data: {
      ...
      endDate: new Date(tomorrow.setHours(16, 0, 0, 0)),
      purpose: 'Practice session for upcoming performance',
      status: 'PENDING',
    },
  })
  ...
  console.log('\n🎉 Seeding completed!')
  console.log('\n📋 Demo Accounts:')
  console.log('- Admin: admin@uic.edu')
  console.log('- Lecturer (Feliks): feliks@uic.edu')
  console.log('- Student: student@uic.edu')
  console.log(`\n📦 Total Equipment: ${allEquipment.length} items`)
}
